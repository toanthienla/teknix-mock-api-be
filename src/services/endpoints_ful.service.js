...
// === BEGIN add-on: keep `id` field even if FE doesn't send it ===
const DEFAULT_ID_FIELD = {
  type: "number",
  required: false,
  readonly: true,
  autogenerated: false,
...
/**
 * Chuẩn hoá schema từ FE (object hoặc array), loại 'id' từ input và ép 'id' mặc định lên đầu.
 * Chấp nhận:
 *  - Object: { name: {type, required}, ... }
 *  - Array:  [ { name, type, required }, ... ]
 */
function normalizeIncomingSchema(input) {
  let obj = {};
  ...
  if (Array.isArray(input)) {
    for (const f of input) {
      ...
      obj[name] = { type: f.type, required: !!f.required };
    }
  } else if (input && typeof input === "object") {
    for (const [k, v] of Object.entries(input)) {
      const name = String(k).trim();
      if (!name || name === "id") continue;
      obj[name] = { type: v?.type, required: !!v?.required };
    }
  }

  // Luôn có 'id' và đứng đầu
  return { id: { ...DEFAULT_ID_FIELD }, ...obj };
}
// === END add-on ===
...
async function updateEndpointData(path, body) {
  ...
  if (schema && data_default) {
    ...
    // 🔒 luôn giữ 'id' dù FE không gửi
    const cloned = JSON.parse(JSON.stringify(data_default));
    const normalizedSchema = normalizeIncomingSchema(schema);
    const withIds = autoAssignIdsIfAllowed(cloned, normalizedSchema);
    const v = validateArrayWithSchema(withIds, normalizedSchema);
...
await statefulPool.query(
      ...
      [JSON.stringify(normalizedSchema), path]
    );
