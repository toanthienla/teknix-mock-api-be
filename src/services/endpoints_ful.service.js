...
const EndpointStatefulService = {
    ...
    async findByFolderId(folderId) {
        const query = 'SELECT * FROM endpoints_ful WHERE folder_id = $1 ORDER BY created_at DESC;';
    ...
    async deleteById(id) {
        ...
        try {
            ...
            const { rows: endpointRows } = await client.query('SELECT path FROM endpoints_ful WHERE id = $1', [id]);
            ...
            if (endpoint.path) {
                await client.query('DELETE FROM endpoint_data_ful WHERE path = $1', [endpoint.path]);
            }
            ...
            console.error(`Transaction failed for deleting stateful endpoint ${id}:`, err);
