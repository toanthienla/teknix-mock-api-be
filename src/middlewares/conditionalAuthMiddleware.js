...
async function conditionalAuthMiddleware(req, res, next) {
  try {
    ...
    // GET requests: bypass auth, let statefulHandler/statelessHandler handle public/private
    if (method === "GET") {
      console.log(`[conditionalAuth] ✅ GET request - bypass auth, handler will check folder.is_public`);
      return next();
    }
