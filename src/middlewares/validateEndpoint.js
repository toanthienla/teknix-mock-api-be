 module.exports = function validateEndpoint(req, res, next) {
  const { name, method, path, folder_id, schema } = req.body || {};
   const errors = [];
  ...
  // ✅ Nếu PUT chỉ cập nhật schema -> bỏ qua các validate khác
  if (req.method === 'PUT' && schema !== undefined && Object.keys(req.body || {}).length === 1) {
    return next();
  }
  ...
  if (req.method === 'POST' || method !== undefined) {
    if (!method) errors.push({ field: "method", message: "Method is required" });
  }

   // validate path

  if (req.method === 'POST' || path !== undefined) {
    if (!path) {
